...
{
  // unit of sigma2s is (degree/hour)^2, unit of tau is second
  if (sigma2s.size() != taus.size())
  {
    std::cerr << "Error of point size" << std::endl;
  }

  m_taus = taus; // unit is second

  std::vector<double> init = initValue(sigma2s, taus);

  int num_samples = sigma2s.size(); // how many points in this allan curve
...
FitAllanGyr::initValue(std::vector<double> sigma2s, std::vector<double> taus)
{
  ...
  {
    Y(index, 0) = sqrt(sigma2s[index]); // unit is degree/hour
  }
  ...
  for (unsigned int index = 0; index < taus.size(); ++index)
  {
    for (int order_index = 0; order_index < 2 * m_order + 1; ++order_index)
    {
      ...
      F(index, order_index) = pow(sqrt(taus[index]), kk); // unit is second^(kk/2)
    }                                                     // endfor: have initialize this row
  }                                                       // endfor: have initialized this matrix
  //        std::cout << "F " << F << std::endl;

  Eigen::MatrixXd A = F.transpose() * F; // A has 2*m_order+1 rows and 2*m_order+1 columns
  B                 = F.transpose() * Y; // B has 2*m_order+1 rows and one column
  //        std::cout << "B " << B << std::endl;
  //        std::cout << "A " << A << std::endl;

  Eigen::MatrixXd C = A.inverse() * B; // C has 2*m_order+1 rows and one column
...
{
  // clang-format off
  return  _Q * _Q / ( _tau * _tau ) // unit: arcsecond*arcsecond/(second*second)=>(arcsecond/second)^2=>(degree/hour)^2
      + _N * _N / _tau
      + _B * _B
      + _K * _K * _tau
      + _R * _R * _tau * _tau;
...
{
  return sqrt(N * N) / 60.0; // equation 8.3-5
}

double
FitAllanGyr::getB() const
{
  return sqrt(B * B) / 0.6642824703; // equation 8.3-10
}

double
FitAllanGyr::getK() const
{
  return 60.0 * sqrt(3.0 * K * K); // equation 8.3-8
}

double
FitAllanGyr::getR() const
{
  return 3600.0 * sqrt(2.0 * R * R); // equation 8.3-12
}

double
FitAllanGyr::getQ() const
{
  return sqrt(Q * Q) / (3600.0 * sqrt(3.0)); // equation 8.3-3
}
