...
{
  // calculate the largest power of 2 (maxstride) that is less than or equal to (numdata/2)
  int mode               = numData / 2;
  unsigned int maxStride = 1; // unit: dimensionless
  int shft               = 0;
  while (mode)
  {
    mode      = mode >> 1;
  ...
  */
  ...
  // std::cout << m_name << " " << " shft: " << shft << std::endl;
...
imu::AllanAcc::getLogSpace(float a, float b)
{
  ...
  double end         = pow(10, b); // considering b=log10(maxStride) --> end=maxStride
  double progression = pow(end / start, (float)1 / (numCluster - 1));

  logSpace.push_back(start);
  for (int i = 1; i < numCluster; i++)
  {
    logSpace.push_back(logSpace[i - 1] * progression); // first is 10^a, last is (10^b)maxStride
  }
  // std::cout << m_name << " " << "size logSpace " << logSpace.size( ) << std::endl;
  return logSpace; // first element is 1, last element is maxStride
}
